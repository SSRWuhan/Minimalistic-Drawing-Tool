...
class mdt(tk.Tk):
    ...
    def __change_resolution(self, resolution, cell_size, screen_dim):
        ...
        with suppress(RecursionError):
            ...
            if int(screen_dim[0]) == int(resolution[1]) * int(cell_size) and int(screen_dim[1]) == int(resolution[0]) * int(cell_size):
                self.__WIDTH = int(screen_dim[0]) 
                self.__HEIGHT = int(screen_dim[1])
                self.__CELL_SIZE = int(cell_size)
                self.__GRID_Y = int(resolution[1])
                self.__GRID_X = int(resolution[0])
    ...
    def __create_querry_window(self):
        ...
        resolution_var = tk.StringVar(value=f"{self.__GRID_X}, {self.__GRID_Y}")
        ...
        screen_dim_var = tk.StringVar(value=f"{self.__WIDTH}, {self.__HEIGHT}") 
